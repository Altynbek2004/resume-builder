...
@section('content')
    <div class="max-w-3xl mx-auto">
        <h1 class="text-3xl font-bold mb-6 text-gray-800">Add Work Experience</h1>
            ...
            <form action="{{ route('experience.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="company" name="company"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('company') border-red-500 @enderror"
                           value="{{ old('company') }}">
                    @error('company')
                ...
                <div>
                    ...
                    <input type="text" id="position" name="position"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('position') border-red-500 @enderror"
                           value="{{ old('position') }}">
                    @error('position')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('location') border-red-500 @enderror"
                           value="{{ old('location') }}">
                    @error('location')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_current" class="ml-2 block text-sm text-gray-700">I am currently working here</label>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    <button type="submit"
                            class="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Add Work Experience</button>
